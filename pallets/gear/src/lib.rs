...
pub enum ProcessStatus {
    ...
    Completed,
    /// Forced to not run or panicked during last run
    SkippedOrFailed,
}
...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        /// The extrinsic that runs queue processing panicked
        QueueProcessingPanicked,
    }
        ...
        T::AccountId: Origin,
    {
        ...
        fn on_finalize(bn: BlockNumberFor<T>) {
            ...
            match QueueState::<T>::get() {
                // Still in `Scheduled` state: last run must have panicked
                ProcessStatus::Scheduled => {
                    // Emitting event to signal queue processing has panicked
                    Self::deposit_event(Event::QueueProcessingPanicked);
                    QueueState::<T>::put(ProcessStatus::SkippedOrFailed);
                }
        ...
        T::AccountId: Origin,
    {
        ...
        #[pallet::weight((
            Weight::from_ref_time(BlockGasLimitOf::<T>::get()),
            DispatchClass::Mandatory
        ))]
        pub fn run_process_queue(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ensure_none(origin)?;
            ...
            let bn = <frame_system::Pallet<T>>::block_number();
            let initial_gas = GasAllowanceOf::<T>::get();
            ...
            let actual_weight = initial_gas.saturating_sub(GasAllowanceOf::<T>::get());
