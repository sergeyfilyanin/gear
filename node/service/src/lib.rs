...
pub fn new_full<RuntimeApi, ExecutorDispatch, Runtime, Extrinsic>(
...
where
    ...
    RuntimeApi::RuntimeApi:
        RuntimeApiCollection<StateBackend = sc_client_api::StateBackendFor<FullBackend, Block>>,
    ExecutorDispatch: NativeExecutionDispatch + 'static,
    Runtime: frame_system::Config + CustomExtrinsic<Extrinsic> + Send + Sync + 'static,
    // Runtime::RuntimeCall: Dispatchable + Clone + Decode + Encode + Send + Sync + 'static,
    ...
    OpaqueExtrinsic: From<Extrinsic>,
{
    ...
    if role.is_authority() {
        let proposer_factory = authorship::ProposerFactory::<_, _, _, _, Runtime, Extrinsic>::new(
            task_manager.spawn_handle(),
            client.clone(),
            transaction_pool,
